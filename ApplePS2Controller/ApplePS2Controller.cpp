...
{
    IOLog("ApplePS2Controller starting\n");
  //
 ...
 if (!super::start(provider))  return false;
  ...
  _interruptSourceMouse    = IOInterruptEventSource::interruptEventSource(
        this, OSMemberFunctionCast(IOInterruptEventAction, this, &ApplePS2Controller::interruptOccurred));
  ...
  _interruptSourceQueue    = IOInterruptEventSource::interruptEventSource(
        this, OSMemberFunctionCast(IOInterruptEventAction, this, &ApplePS2Controller::processRequestQueue));
  ...
  _mouseDevice->registerService();
    IOLog("ApplePS2Controller started\n");
  return true; // success
                                                ...
                                                PS2InterruptAction action)
{
  ...
  if (deviceType == kDT_Keyboard && _interruptInstalledKeyboard == false)
  {
    ...
    _workLoop->addEventSource(_interruptSourceKeyboard);
      _interruptSourceKeyboard->enable();
                                                  ...
                                                  void * arg2, void * arg3 )
{
  ...
  uintptr_t powerState = reinterpret_cast<uintptr_t>(arg0);
...
void ApplePS2Controller::setPowerStateGated( uintptr_t powerState )
{
  UInt8 commandByte;
